	...
	return function(selector) {
		...
		var createOptionOrGroup = function(element, index) {
			...
			if (element.tagName.toUpperCase() === 'OPTGROUP') {
				...
				if (isGroupClosed) {
					isGroupClosed = false;
					result += '<div class="ss-group" data-label="default">';
				}
		...
		var createOption = function(realOption, index) {
			...
			if (index === selectedIndex) {
				...
				selectedOptionHTML = '<div class="ss-selected-option" tabindex="0" data-value="' + value + '">' + (realSelect.getAttribute('placeholder') || text) + '</div>'
			}
